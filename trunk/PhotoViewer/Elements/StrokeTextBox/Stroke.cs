...
namespace PhotoViewer.Element
{
    public class Stroke
    {
        ...
        //public void changeTagBox()
        //{
        //    Vector2 size = boundingbox.Max - boundingbox.Min;
        //    boundingbox = new BoundingBox2D(Strokes[Strokes.Count - 2], Strokes[Strokes.Count - 2] + size, 0);
        //}
        ...
        public Vector2 renderTag()
        {
            if (this.tags_.Count > 0)
            {
                ...
                Browser.Instance.batch_.Draw(ResourceManager.frameSquare_, new Rectangle((int)this.Strokes[0].X, (int)this.Strokes[0].Y, width, height), Color.White);
                width = 5;
                foreach (var tag in tags_)
                {
                    Browser.Instance.batch_.DrawString(ResourceManager.font_, tag, new Vector2(this.Strokes[0].X + width, strokes_[0].Y), this.Color);
                    width += (int)ResourceManager.font_.MeasureString(tag).X + 5;
                }
