...
namespace PhotoInfo
{
    public class PhotoTag //: IComparable
    {
        ...
        //for language
        public String SoundexEng = null;
        public String SoundexRomajic = null;
        ...
        public int drawEng(int startX, int startY, float angleDisplay)
        {
            int resultWidth = 0;
            int y = 0;
            if (English != null)
            {
                Browser.Instance.batch_.DrawString(ResourceManager.font_, English, new Vector2(startX + left, startY), Color.Red);
                resultWidth = (int)ResourceManager.font_.MeasureString(English).Length() + 5;
                y = (int)ResourceManager.font_.MeasureString(English).Y + 5;
                //Browser.Instance.batch_.DrawString(ResourceManager.font_, rank.ToString(), new Vector2(startX + left, startY + y), Color.Gray);
                //Browser.Instance.batch_.DrawString(ResourceManager.font_, this.SoundexEng, new Vector2(startX + left, startY + 2 * y), Color.Gray);
            }
            return resultWidth;
        }
        ...
        public List<int> drawTags(int startX, int startY, float angleDisplay)
        {
            ...
            if (allTags.Count > 1)
            {
                ...
                //Browser.Instance.batch_.DrawString(ResourceManager.font_, this.SoundexRomajic, new Vector2(startX + left, startY + YOffset), Color.Red);
