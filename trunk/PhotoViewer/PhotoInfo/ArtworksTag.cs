...
namespace PhotoInfo
{
    public class ArtworksTag : PhotoTag
    {
        ...
        public class LanEntity
        {
            ...
            public string SoundexRomanji
            {
                get;
                set;
            }

            public LanEntity(List<string> tag, string sen, string ro)
            {
                //English = eng;
                //Japanese = ja;
                tags = tag;
        ...
        public static Dictionary<string, PhotoTag> FromXml(string xml)
        {
            XmlSerializer ser = new XmlSerializer(typeof(LanEntity[]));
            var tags = (LanEntity[])ser.Deserialize(new StringReader(xml));
            Dictionary<string, PhotoTag> lantags = new Dictionary<string, PhotoTag>();
            foreach (var tag in tags)
            {
                lantags[tag.tags[0]] = new LanTag(tag.tags, tag.SoundexEng, tag.SoundexRomanji);
            }
            return lantags;
        }

        public static string ExportXml(Dictionary<string, PhotoTag> tag)
        {
            List<LanEntity> list = new List<LanEntity>();
            foreach (var t in tag.Keys)
            {
                list.Add(new LanEntity(tag[t].allTags, tag[t].SoundexEng, tag[t].SoundexRomajic));
            }
            XmlSerializer ser = new XmlSerializer(typeof(LanEntity[]));
