...
namespace PhotoInfo
{
    public class Photo : IEnumerable
    {
        ...
        public void LanShowEng()
        {
            maxLanLength = ptag.drawEng((int)(this.PositionDisplay.X + this.WidthDisplay / 2 + Browser.MAR), (int)(this.PositionDisplay.Y - this.HeightDisplay / 2), angleDisplay_);
        }
        ...
        public void End()
        {
            ...
            else if (IsFollowing && maxLanLength > 0)
            {
                boundingBox_ = new BoundingBox2D(
                  position_ - center_ * scale_ - Vector2.One * (float)((1d - Math.Exp(-(double)scale_)) * (double)Browser.MAR + 2d),
                  position_ + center_ * scale_ + Vector2.One * (float)((1d - Math.Exp(-(double)scale_)) * (double)Browser.MAR + 2d) + new Vector2(maxLanLength , 0), angle_);
                boundingBoxDisplay_ = new BoundingBox2D(
                    positionDisplay_ - center_ * scaleDisplay_ - Vector2.One * (float)((1d - Math.Exp(-(double)scaleDisplay_)) * (double)Browser.MAR + 2d),
                    positionDisplay_ + center_ * scaleDisplay_ - Vector2.One * (float)((1d - Math.Exp(-(double)scaleDisplay_)) * (double)Browser.MAR + 2d) + new Vector2(maxLanLength , 0), angleDisplay_);
        ...
        //FloatTextBox textBox = null;//new FloatTextBox();
        //System.Windows.Forms.Button b = new System.Windows.Forms.Button();

        //public void DrawTextbox()
        //{
        //    //FloatTextBox textBox = new FloatTextBox();
        //    if (textBox == null)
        //        textBox = new FloatTextBox();
        //    textBox.showAgain(new Vector2(positionDisplay_.X - WidthDisplay/2, positionDisplay_.Y + HeightDisplay/2));
            
        //    //b.Location = new System.Drawing.Point(textBox.Location.X + textBox.Width, textBox.Location.Y);
        ...
        public void Render(SpriteBatch batch, bool isFiltering, bool isShadowing, Texture2D textureShadow,bool isWhiteframe, bool isDragged, Texture2D textureFrame, bool isFavorite)
        {
            ...
            //Browser.Instance.batch_.Begin(SpriteSortMode.BackToFront, BlendState.AlphaBlend);
            try
            {
                if (isFiltering)
                {
                            ...
                            if (textureFrame != null)
                            {
                                ...
                                if (isDragged)
                                {
                                    batch.Draw(textureFrame, positionDisplay_, null, Microsoft.Xna.Framework.Color.LightGray,
                                      angleDisplay_, Vector2.One, scaleFrame, SpriteEffects.None, layerDepth_ + ep);//Math.Min(1.0f, layerDepth_ + ep)
                                }
                                //else if (IsFollowing)
                                //{
                                //    batch.Draw(textureFrame, positionDisplay_, null, Microsoft.Xna.Framework.Color.Pink,
                                //      angleDisplay_, Vector2.One, scaleFrame, SpriteEffects.None, layerDepth_ + ep);//Math.Min(1.0f, layerDepth_ + ep)
                ...
                //if (IsGazeds == false && textBox != null && textBox.IsShown)
                  //  textBox.PostTags();
                if (IsFollowing)
                {
                    LanShowEng();
