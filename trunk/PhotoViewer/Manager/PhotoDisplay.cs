    ...
    class PhotoDisplay
    {
        ...
        public void photoBehavior()
        {
            ...
            #region time state x control
                    ...
                    float va = 0f;
#endif
//            if (systemState.IsTime && systemState.IsBound)
//            {
//                int Xth = (int)Math.Sqrt((double)(Browser.Instance.ClientWidth * Browser.Instance.ClientHeight) / (double)photos.Count);
//                // X方向の画面外に出ないようにする制約をキャンセル
//                foreach (Photo photo in photos)
//                {
//                    Vector2 v = Vector2.Zero;
//#if NO_ROTATION
//                    if (photo.BoundingBox.Min.X < dock.DockBoundX - Xth)
//                    {
//                        v.X += (photo.BoundingBox.Min.X - dock.DockBoundX) * 30f;
//                    }
//                    if (photo.BoundingBox.Max.X > Browser.Instance.ClientWidth + Xth)
//                    {
//                        v.X += (photo.BoundingBox.Max.X - Browser.Instance.ClientWidth) * 30f;
//                    }
//#else
//                        for (int i = 0; i < 4; ++i)
//                        {
//                            Vector2 v1 = photo.Position - photo.BoundingBox.Vertex[i];
//                            v1.Normalize();
//                            Vector2 v2 = Vector2.Zero;
//                            float dist = 0;
//                            if (photo.BoundingBox.Vertex[i].X < dock_.DockBoundX - Xth)
//                            {
//                                v2 = Vector2.UnitX;
//                                dist = dock_.DockBoundX - photo.BoundingBox.Vertex[i].X;
//                                //v -= v1 * (float)(dist * Math.Abs(v1.X * v2.X + v1.Y * v2.Y)) * 20f;
//                                v -= dist * v2 * 20f;
//                                va += (v1.X * v2.Y - v1.Y * v2.X) * 10f;
//                            }
//                            if (photo.BoundingBox.Vertex[i].X > input_.WindowWidth + Xth)
//                            {
//                                v2 = -Vector2.UnitX;
//                                dist = photo.BoundingBox.Vertex[i].X - input_.WindowWidth;
//                                //v -= v1 * (float)(dist * Math.Abs(v1.X * v2.X + v1.Y * v2.Y)) * 20f;
//                                v -= dist * v2 * 20f;
//                                va += (v1.X * v2.Y - v1.Y * v2.X) * 10f;
//                            }
