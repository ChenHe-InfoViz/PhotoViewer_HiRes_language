    ...
    class MouseController
    {
                ...
                if (strokeUnderDrawing.ContainsKey(pd))
                {
                    ...
                    if (!s.IsClosed)
                    {
                        if (s.Strokes.Count > 2)
                        {
                            ...
                            //strokeGroup.photoInStroke[s] = new List<Photo>();
                        }
                        else
                        {
                            // 必须少于2个锚点
        ...
        void moveLine(PointingDevice pd)
        {
            ...
            if (strokeMouse.ContainsKey(pd.Header))
            {
                ...
                {
                    //strokeMouse[pd.Header].changeBox();
        ...
        void iconMouse(Icon icon, PointingDevice pd)
        {
            ...
            if (pd.LeftButton == ButtonState.Pressed && pd.oldLeftButton == ButtonState.Released)
            {
                ...
                else if (icon.Attractor == SystemState.FILE_OPEN)
                {
                    ...
                    {
                        //photos_.Clear();
                        String[] tempText = dialog.fileNames[0].Split(new Char[] { '\\' }, StringSplitOptions.RemoveEmptyEntries);
                        if (tempText.Length - 2 >= 0)
                        {
                            if (tempText[tempText.Length - 2] == "language")
                            {
                                systemState.pType = SystemState.PICTURE.LAN;
                                createPhoto = new LanguageCreator();
                            }
                            else if (tempText[tempText.Length - 2] == "artworks")
                            {
                                systemState.pType = SystemState.PICTURE.ART;
                                createPhoto = new ArtworksCreator();
                            }
                            else createPhoto = new PhotoCreator();
                        }
